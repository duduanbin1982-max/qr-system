...
PROCESS_REFERENCES = (
    ...
    _process(
        ...
        key="schedule_revision_conflicts",
        label="排程版本冲突证据",
        impact_level=IMPACT_INTERNAL,
        action="保留不可变冲突检测证据",
    ),
    _process(
        "schedule_replan_triggers",
        ("order_process_id",),
        key="schedule_replan_triggers",
        label="动态重排触发证据",
        impact_level=IMPACT_INTERNAL,
        action="保留不可变生产事实变化证据",
    ),
    _process(
        "schedule_replan_differences",
        ("order_process_id", "process_id"),
        key="schedule_replan_differences",
        label="动态重排前后差异",
        impact_level=IMPACT_INTERNAL,
        action="保留不可变重排差异证据",
    ),
