  ...
  <div class="card" style="border-radius:var(--radius-lg);overflow:hidden;padding:0">
    ...
    <div v-else-if="monthlyData.summary.length" style="padding:0 20px 20px">
      <div class="summary-bar" style="margin-top:12px">
        <div class="summary-item"><span class="s-icon">👥</span><div><div class="s-val text-primary">{{ monthlyTotal }}</div><div class="s-label">员工数(全月)</div></div></div>
        <div class="summary-item"><span class="s-icon">📦</span><div><div class="s-val text-success">{{ monthlyData.grand_total_quantity }}</div><div class="s-label">总件数(全月)</div></div></div>
        <div class="summary-item"><span class="s-icon">💰</span><div><div class="s-val" style="color:var(--warning)">¥{{ fmtMoney(monthlyData.grand_total_wage) }}</div><div class="s-label">总工资(全月)</div></div></div>
      </div>
      <table style="width:100%;border-collapse:collapse;margin-top:12px">
        <thead><tr style="border-bottom:1px solid var(--border-light);color:var(--text-placeholder);font-size:var(--text-xs)">
          <th style="padding:var(--space-3) 12px;text-align:left;font-weight:500;width:40px">排名</th>
          ...
          <th style="padding:var(--space-3) 12px;text-align:left;font-weight:500;width:140px">占比(全月)</th>
        </tr></thead>
        <tbody>
          <tr v-for="(s,i) in monthlyData.summary" :key="s.user_id||i" style="border-bottom:1px solid var(--bg-hover);font-size:var(--text-sm)">
            <td style="padding:var(--space-3) 12px;color:var(--text-placeholder)">{{ (monthlyPage-1)*monthlyLimit + i+1 }}</td>
            ...
            <td style="padding:var(--space-3) 12px">
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <span style="font-size:var(--text-xs);color:var(--text-placeholder);white-space:nowrap;min-width:45px;text-align:right">{{ monthlyPercent(s.total_wage) }}%</span>
              </div>
            </td>
          </tr>
        </tbody>
