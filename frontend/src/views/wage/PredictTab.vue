...
<div v-if="activeTab==='predict'">
      <div class="card" style="border-radius:var(--radius-lg);overflow:hidden;padding:0">
        ...
        <div v-else-if="predError" style="text-align:center;padding:60px;color:var(--danger)">⚠️ {{ predError }}</div>
        <div v-else-if="!predData.months_data || predData.months_data < 2" style="text-align:center;padding:60px;color:var(--text-placeholder)">
        ...
        <div v-else style="padding:20px">
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px">
            ...
            <div class="card" style="padding:16px;text-align:center;background:linear-gradient(135deg,var(--bg-secondary),var(--bg))">
              <div style="font-size:var(--text-xs);color:var(--text-placeholder);margin-bottom:4px">预测产量</div>
              <div style="font-size:28px;font-weight:700;color:var(--text-primary)">{{ predData.predicted_quantity || 0 }}</div>
            </div>
