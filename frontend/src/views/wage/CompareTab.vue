        ...
        <tbody>
          <tr v-for="c in compareData" :key="c.employee_name + c.wageA + c.wageB" style="border-bottom:1px solid var(--bg-hover);font-size:var(--text-sm)">
            ...
            <td style="padding:var(--space-3) 12px">
              <template v-if="c.wageA===0 && c.wageB>0">
                <span style="color:var(--success);font-weight:600">🆕 新增</span>
              </template>
              <template v-else-if="c.wageB===0 && c.wageA>0">
                <span style="color:var(--danger);font-weight:600">🚫 离职</span>
              </template>
              <template v-else-if="c.change>0">
                <span style="color:var(--success)">📈 ↑{{ c.changePct }}%</span>
              </template>
              <template v-else-if="c.change<0">
                <span style="color:var(--danger)">📉 ↓{{ Math.abs(c.changePct) }}%</span>
              </template>
              <template v-else>
                <span style="color:var(--text-placeholder)">— 持平</span>
              </template>
            </td>
          </tr>
        </tbody>
