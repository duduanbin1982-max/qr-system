    ...
    <div class="card pqe-shell">
      <div class="card-header pqe-toolbar">
        ...
        <div class="pqe-actions">
          ...
          <select v-if="activeTab === 'disposal'" v-model="disposalStatus" class="form-input pqe-status-filter" @change="loadDisposal"><option value="pending">待处置</option><option value="waived">已豁免</option><option value="audit">永久审计</option></select>
...
const showWaiver = ref(false), waiving = ref(false), waiverForm = reactive({ taskIds: [], orderId: null, orderNo: '', scope: '', reasonCode: '', reason: '' })
...
const waiverReasonOptions = computed(() => waiverForm.scope === 'live' ? (waiverPolicy.value.live_reasons || []) : (waiverPolicy.value.historical_reasons || []))
...
const waiverTargetLabel = computed(() => { const target = waiverForm.orderId ? `订单 ${waiverForm.orderNo || waiverForm.orderId} 的全部待处理必评任务` : `已选择的 ${waiverForm.taskIds.length} 条待评价任务`; return waiverForm.scope === 'live' ? `${target}属于生产中或待生产订单，豁免后将立即解除报工门禁，请确认已取得授权。` : `${target}将按历史任务留痕处置。` })
...
async function loadDisposal() { const taskRequest = disposalStatus.value === 'audit' ? api.domains.processQualityEvaluations.qualityEvaluationTaskAudits({ keyword: keyword.value, per_page: 500 }) : api.domains.processQualityEvaluations.qualityEvaluationTasks({ scope: 'all', status: disposalStatus.value, keyword: keyword.value, per_page: 500 }); const [summary, taskData] = await Promise.all([api.domains.processQualityEvaluations.qualityEvaluationTaskDisposalSummary(), taskRequest]); disposalSummary.value = summary || {}; disposalTasks.value = taskData.items || []; const visibleIds = new Set(disposalTasks.value.filter(task => task.status === 'pending').map(task => task.id)); selectedTaskIds.value = selectedTaskIds.value.filter(id => visibleIds.has(id)) }
...
function orderStatusText(task) { if (task.order_deleted_at) return '已归档'; return { completed: '已完成', cancelled: '已取消', pending: '待生产', producing: '生产中', paused: '已暂停' }[task.order_status] || task.order_status || '-' }
...
function openWaiver(taskIds) { const ids = [...new Set(taskIds)]; const selected = disposalTasks.value.filter(task => ids.includes(task.id)); const scopes = [...new Set(selected.map(waiverScopeForTask))]; if (scopes.length !== 1) { showToast('生产中订单与历史订单请分开处置', 'error'); return } if (scopes[0] === 'live' && !canWaiveLive.value) { showToast('您没有生产中订单豁免权限', 'error'); return } Object.assign(waiverForm, { taskIds: ids, orderId: null, orderNo: '', scope: scopes[0], reasonCode: '', reason: '' }); showWaiver.value = true }
function openOrderWaiver(task) { const scope = waiverScopeForTask(task); if (scope === 'live' && !canWaiveLive.value) { showToast('您没有生产中订单豁免权限', 'error'); return } Object.assign(waiverForm, { taskIds: [], orderId: task.order_id, orderNo: task.order_no, scope, reasonCode: '', reason: '' }); showWaiver.value = true }
async function saveWaiver() { if (!waiverForm.reasonCode) { showToast('请选择豁免原因类型', 'error'); return } const minimumLength = waiverForm.scope === 'live' ? 10 : 2; if (waiverForm.reason.trim().length < minimumLength) { showToast(`请填写至少${minimumLength}个字符的豁免说明`, 'error'); return } waiving.value = true; try { const payload = { reason_code: waiverForm.reasonCode, reason: waiverForm.reason.trim() }; if (waiverForm.orderId) { payload.order_id = waiverForm.orderId; payload.required_only = true } else payload.task_ids = waiverForm.taskIds; const result = await api.domains.processQualityEvaluations.waiveQualityEvaluationTasks(payload); showWaiver.value = false; selectedTaskIds.value = []; showToast(`已豁免 ${result.count || 0} 条评价任务`); await loadDisposal(); await loadTasks() } catch (error) { showToast(error.message || '豁免失败', 'error') } finally { waiving.value = false } }
