    ...
    <div class="card pqe-shell">
      <div class="card-header pqe-toolbar">
        ...
        <div class="pqe-actions">
          ...
          <select v-if="activeTab === 'disposal'" v-model="disposalStatus" class="form-input pqe-status-filter" @change="onDisposalStatusChange"><option value="pending">待处置</option><option value="waived">已豁免</option><option value="audit">永久审计</option></select>
      ...
      <div class="card-body pqe-body">
        ...
        <section v-else-if="activeTab === 'disposal'">
          ...
          <div class="pqe-disposal-summary">
            ...
            <div><strong>{{ disposalSummary.overdue_24h || 0 }}</strong><span>必评超24小时</span></div>
            <div><strong class="text-danger">{{ disposalSummary.overdue_72h || 0 }}</strong><span>必评超72小时</span></div>
          ...
          <div class="pqe-disposal-actions">
            <label v-if="disposalStatus === 'pending'"><input type="checkbox" :checked="allPendingSelected" @change="toggleAllDisposal($event.target.checked)"> 全选当前可处置任务</label>
...
const disposalPage = ref(1), disposalTotal = ref(0), disposalStatus = ref('pending')
const statsSummary = ref({}), appealSummary = ref({}), processStats = ref([]), evaluatorStats = ref([])
const disposalSummary = ref({}), selectedTaskIds = ref([])
...
const waiverPolicy = computed(() => disposalSummary.value.waiver_policy || {})
const canWaiveLive = computed(() => Boolean(waiverPolicy.value.can_waive_live))
const taskPages = computed(() => pageCount(taskTotal.value))
const disposalPages = computed(() => pageCount(disposalTotal.value))
const recordPages = computed(() => pageCount(recordTotal.value))
const disposableTasks = computed(() => disposalTasks.value.filter(task => task.status === 'pending' && canWaiveTask(task)))
const allPendingSelected = computed(() => disposalStatus.value === 'pending' && disposableTasks.value.length > 0 && selectedTaskIds.value.length === disposableTasks.value.length)
...
const disposalEmptyText = computed(() => ({ pending: '待处置', waived: '已豁免', audit: '永久审计' }[disposalStatus.value] || ''))
...
async function loadDisposal() { const params = { keyword: keyword.value, page: disposalPage.value, per_page: PAGE_SIZE }; const taskRequest = disposalStatus.value === 'audit' ? api.domains.processQualityEvaluations.qualityEvaluationTaskAudits(params) : api.domains.processQualityEvaluations.qualityEvaluationTasks({ ...params, scope: 'all', status: disposalStatus.value }); const [summary, taskData] = await Promise.all([api.domains.processQualityEvaluations.qualityEvaluationTaskDisposalSummary(), taskRequest]); const lastPage = pageCount(taskData.total); if (disposalPage.value > lastPage) { disposalPage.value = lastPage; return loadDisposal() } disposalSummary.value = summary || {}; disposalTasks.value = taskData.items || []; disposalTotal.value = taskData.total || 0; const visibleIds = new Set(disposalTasks.value.filter(task => task.status === 'pending').map(task => task.id)); selectedTaskIds.value = selectedTaskIds.value.filter(id => visibleIds.has(id)) }
...
function resetActivePage() { if (activeTab.value === 'tasks') taskPage.value = 1; if (activeTab.value === 'disposal') disposalPage.value = 1; if (['records', 'review'].includes(activeTab.value)) recordPage.value = 1 }
async function reloadActiveFromFirstPage() { resetActivePage(); await loadActive() }
async function switchTab(key) { activeTab.value = key; resetActivePage(); selectedTaskIds.value = []; localStorage.setItem('processQualityEvaluationTab', key); await loadActive() }
async function onDisposalStatusChange() { disposalPage.value = 1; selectedTaskIds.value = []; await loadDisposal() }
async function changeTaskPage(delta) { taskPage.value += delta; await loadTasks() }
async function changeDisposalPage(delta) { disposalPage.value += delta; selectedTaskIds.value = []; await loadDisposal() }
...
function toggleAllDisposal(checked) { selectedTaskIds.value = checked ? disposableTasks.value.map(task => task.id) : [] }
...
function orderStatusLabel(status) { return { completed: '已完成', cancelled: '已取消', pending: '待生产', producing: '生产中', paused: '已暂停' }[status] || status || '-' }
...
async function openWaiver(taskIds) { const ids = [...new Set(taskIds)]; await openWaiverPreview({ taskIds: ids, orderId: null, orderNo: '' }) }
async function openOrderWaiver(task) { await openWaiverPreview({ taskIds: [], orderId: task.order_id, orderNo: task.order_no }) }
async function saveWaiver() { if (!waiverPreview.value?.can_submit) { showToast('当前选择不能提交，请根据风险提示调整任务范围', 'error'); return } if (!waiverForm.reasonCode) { showToast('请选择豁免原因类型', 'error'); return } const minimumLength = waiverForm.scope === 'live' ? 10 : 2; if (waiverForm.reason.trim().length < minimumLength) { showToast(`请填写至少${minimumLength}个字符的豁免说明`, 'error'); return } waiving.value = true; try { const payload = { ...waiverPayload(), reason_code: waiverForm.reasonCode, reason: waiverForm.reason.trim() }; const result = await api.domains.processQualityEvaluations.waiveQualityEvaluationTasks(payload); showWaiver.value = false; waiverPreview.value = null; selectedTaskIds.value = []; showToast(`已豁免 ${result.count || 0} 条评价任务`); await loadDisposal(); await loadTasks() } catch (error) { showToast(error.message || '豁免失败', 'error') } finally { waiving.value = false } }
...
.pqe-pagination{display:flex;align-items:center;justify-content:flex-end;gap:var(--space-3);padding-top:var(--space-3);color:var(--text-secondary);font-size:var(--text-sm)}.pqe-waiver-impact{margin-top:var(--space-3);border:1px solid var(--border)}.pqe-impact-summary{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid var(--border)}.pqe-impact-summary>div{padding:10px 12px;text-align:center}.pqe-impact-summary strong{display:block;font-size:var(--text-lg)}.pqe-impact-summary span{color:var(--text-placeholder);font-size:var(--text-xs-alt)}.pqe-impact-orders{max-height:160px;overflow:auto}.pqe-impact-orders>div{display:grid;grid-template-columns:minmax(130px,1fr) 80px minmax(180px,1.4fr);gap:var(--space-2);padding:8px 12px;border-bottom:1px solid var(--border);font-size:var(--text-sm)}.pqe-impact-orders>div:last-child{border-bottom:0}.pqe-impact-warnings{margin:0;padding:10px 30px;border-top:1px solid var(--border);background:var(--warning-light);color:var(--text-secondary);font-size:var(--text-sm)}
@media(max-width:900px){.pqe-page{padding:var(--space-3)}.pqe-actions{margin-left:0;width:100%}.pqe-search{width:min(100%,240px)}.pqe-disposal-summary,.pqe-stats-grid,.pqe-rule-grid,.pqe-form-grid{grid-template-columns:1fr}.pqe-disposal-actions{align-items:flex-start;flex-direction:column}.pqe-rule-grid label{grid-template-columns:1fr 130px}.pqe-pagination{justify-content:flex-start;flex-wrap:wrap}.pqe-impact-summary{grid-template-columns:repeat(2,1fr)}.pqe-impact-orders>div{grid-template-columns:1fr}}
.pqe-live-warning{border:1px solid var(--danger);background:var(--danger-light);color:var(--danger)}
</style>
